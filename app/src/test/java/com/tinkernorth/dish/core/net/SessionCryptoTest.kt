...
class SessionCryptoTest {
    ...
    fun `hmacProof matches the satellite's pinned vector`() {
        assertEquals(
            ...
            hmacProof(pairingKey, "device-1"),
        )
    }

    @Test
    fun `hmacProof is device-bound and key-bound`() {
        val p1 = hmacProof(pairingKey, "device-1")
        assertNotEquals(p1, hmacProof(pairingKey, "device-2"))
        val otherKey = pairingKey.copyOf().also { it[0] = 0x7F }
        assertNotEquals(p1, hmacProof(otherKey, "device-1"))
    ...
    fun `deriveSessionKey matches the satellite's pinned HKDF vector`() {
        ...
        val key = deriveSessionKey(pairingKey, salt, token)
        assertEquals(
    ...
    fun `deriveSessionKey never returns the raw pairing key and varies with inputs`() {
        ...
        val k1 = deriveSessionKey(pairingKey, salt, token)
        assertNotEquals(pairingKey.toList(), k1.toList())
        val k2 = deriveSessionKey(pairingKey, salt, byteArrayOf(0, 0, 0, 2))
        assertNotEquals(k1.toList(), k2.toList())
        val k3 = deriveSessionKey(pairingKey, ByteArray(8) { 2 }, token)
    ...
    fun `deriveSessionKey rejects malformed inputs instead of deriving garbage`() {
        assertThrows(IllegalArgumentException::class.java) {
            deriveSessionKey(ByteArray(16), ByteArray(8), ByteArray(4))
        }
        assertThrows(IllegalArgumentException::class.java) {
            deriveSessionKey(pairingKey, ByteArray(4), ByteArray(4))
        }
        assertThrows(IllegalArgumentException::class.java) {
