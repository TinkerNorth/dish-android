...
class MoonlightInputEncoderTest {
    ...
    fun `CONTROLLER_MULTI splits high buttons into buttonFlags2`() {
        ...
        buf.position(20)
        assertEquals(BTN_A, buf.short.toInt() and 0xFFFF) // low flags
        ...
        assertEquals(BTN_PADDLE1 ushr 16, buf.short.toInt() and 0xFFFF)
    }
    ...
    fun `hot-path encode into a reused buffer matches the allocating form`() {
        ...
        val writer = ControllerMultiWriter(reused)
        writer.encode(0, 1, BTN_B, 0, 0, 0, 0, 0, 0)
        ...
        writer.encode(0, 1, BTN_A, 0, 0, 0, 0, 0, 0)
        val second = ByteArray(reused.remaining()).also { reused.get(it) }
        assertEquals(CONTROLLER_MULTI_LEN, first.size)
        assertEquals(
    ...
    fun `MOUSE_SCROLL duplicates the big-endian amount and pads a zero i16`() {
        val up = mouseScroll(120)
        assertEquals(MOUSE_SCROLL_LEN, up.size)
        assertEquals("0602" + "0e00" + "0000000a" + "0a000000" + "0078" + "0078" + "0000", bytesToHex(up))
        val down = mouseScroll(-120)
    ...
    fun `CONTROLLER_ARRIVAL carries type and capabilities`() {
        val bytes =
            controllerArrival(
                controllerNumber = 0,
                controllerType = CONTROLLER_TYPE_XBOX,
                capabilities = CAP_ANALOG_TRIGGERS or CAP_RUMBLE,
                supportedButtons = 0xFFFF,
            )
        assertEquals(CONTROLLER_ARRIVAL_LEN, bytes.size)
        val buf = ByteBuffer.wrap(bytes).order(ByteOrder.LITTLE_ENDIAN)
        assertEquals(CTRL_INPUT_DATA, buf.short.toInt() and 0xFFFF)
        buf.short // plen
        ...
        assertEquals(CONTROLLER_TYPE_XBOX, buf.get().toInt())
    ...
    fun `CONTROLLER_TOUCH is byte-exact against the Wolf packed struct`() {
        ...
        assertEquals(CONTROLLER_TOUCH_LEN, bytes.size)
        assertEquals(
    ...
    fun `CONTROLLER_MOTION is byte-exact with gyro floats in deg per second`() {
        ...
        assertEquals(CONTROLLER_MOTION_LEN, bytes.size)
        assertEquals(
