...
class CompatPillTest {
    ...
    fun `a still-working older satellite gets the soft amber chip`() {
        assertEquals(
            ...
            compatPillParts(DishProtocolCompat.SATELLITE_UPDATE_AVAILABLE),
        )
    }
    ...
    fun `hard blocks get the error chip naming the side that must update`() {
        assertEquals(
            ...
            compatPillParts(DishProtocolCompat.SATELLITE_UPDATE_REQUIRED),
        )
        assertEquals(
            R.string.chip_app_update_required to PillTone.ERROR,
            compatPillParts(DishProtocolCompat.APP_UPDATE_REQUIRED),
        )
    }

    @Test
    fun `current and unknown render nothing`() {
        assertNull(compatPillParts(DishProtocolCompat.CURRENT))
        assertNull(compatPillParts(DishProtocolCompat.UNKNOWN))
    }
}
