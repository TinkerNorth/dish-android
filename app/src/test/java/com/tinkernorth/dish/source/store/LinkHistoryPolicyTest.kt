...
class LinkHistoryPolicyTest {
    ...
    fun `a link coming up counts one connect and remembers when`() {
        val s1 = onConnections(LinkHistoryState(), listOf(summary("a", LinkState.Saved)), nowMs = 10)
        assertEquals(0, s1.connections.getValue("a").connects)
        assertNull(s1.connections.getValue("a").upSinceMs)

        val s2 = onConnections(s1, listOf(summary("a", LinkState.Connected)), nowMs = 20)
        assertEquals(1, s2.connections.getValue("a").connects)
        assertEquals(20L, s2.connections.getValue("a").upSinceMs)

        val s3 = onConnections(s2, listOf(summary("a", LinkState.Unstable)), nowMs = 30)
    ...
    fun `a link going down counts a drop and clears the uptime`() {
        val up = onConnections(LinkHistoryState(), listOf(summary("a", LinkState.Connected)), nowMs = 20)
        val down = onConnections(up, listOf(summary("a", LinkState.Saved)), nowMs = 50)
    ...
    fun `a binding remembers when it was made and resets when it moves host`() {
        val s1 = onBindings(LinkHistoryState(), mapOf("7" to "a"), nowMs = 100)
        assertEquals(100L, s1.boundSinceMs["7"])

        val s2 = onBindings(s1, mapOf("7" to "a"), nowMs = 200)
        assertEquals(100L, s2.boundSinceMs["7"])

        val s3 = onBindings(s2, mapOf("7" to "b"), nowMs = 300)
        assertEquals(300L, s3.boundSinceMs["7"])

        val s4 = onBindings(s3, emptyMap(), nowMs = 400)
