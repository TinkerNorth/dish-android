...
class FrameworkTimingWindowTest {
    ...
    fun `percentiles use nearest rank`() {
        ...
        assertEquals(3f, Percentiles.percentileOf(values, 0.5), 0f)
        assertEquals(5f, Percentiles.percentileOf(values, 0.99), 0f)
        assertEquals(0f, Percentiles.percentileOf(FloatArray(0), 0.5), 0f)
    }
