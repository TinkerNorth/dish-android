...
class WifiConnectionTest {
    ...
    private val server = DiscoveredServer(
        name = "Pc", ip = "10.0.0.5", udpPort = 9876, pairPort = 9878, httpPort = 9877,
    )
    ...
    @Test
    fun `markConnected stores handle, id and starts heartbeat + ack reset`() = runTest {
    ...
    @Test
    fun `attachSlot records the binding even when not connected`() = runTest {
    ...
    @Test
    fun `attachSlot when already connected registers controller with server`() = runTest {
        every { repo.resetControllerAck(any()) } just Runs
        every { repo.startHeartbeat(any()) } just Runs
    ...
    @Test
    fun `markConnected preserves a slot binding recorded before connect`() = runTest {
        every { repo.resetControllerAck(any()) } just Runs
        every { repo.startHeartbeat(any()) } just Runs
        every { repo.isConnectionAlive(any()) } returns false
