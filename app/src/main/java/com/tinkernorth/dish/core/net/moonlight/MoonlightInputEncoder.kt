...
class ControllerMultiWriter(
    ...
    fun encode(
        ...
        dst.putInt(INPUT_CONTROLLER_MULTI)
        // Struct body (all little-endian).
        dst.putShort(MULTI_HEADER_B.toShort())
        dst.putShort(controllerNumber.toShort())
        dst.putShort(activeMask.toShort())
        dst.putShort(MULTI_MID_B.toShort())
        ...
        dst.putShort(MULTI_TAIL_A.toShort())
        dst.putShort((buttons ushr 16).toShort())
        dst.putShort(MULTI_TAIL_B.toShort())
        dst.flip()
    }
}
...
fun controllerArrival(
    ...
    buf.putInt(INPUT_CONTROLLER_ARRIVAL)
...
fun controllerMotion(
    ...
    buf.putInt(INPUT_CONTROLLER_MOTION)
    buf.put((controllerNumber and 0xFF).toByte())
...
fun controllerBattery(
    ...
    buf.putInt(INPUT_CONTROLLER_BATTERY)
    buf.put((controllerNumber and 0xFF).toByte())
...
fun periodicPing(): ByteArray {
    ...
    buf.putShort(CTRL_PERIODIC_PING.toShort())
...
fun termination(): ByteArray {
    ...
    buf.putShort(CTRL_TERMINATION.toShort())
    buf.putShort((TERMINATION_LEN - CONTROL_HEADER_LEN).toShort())
    putIntBE(buf, TERMINATE_REASON_GRACEFUL)
    return buf.toByteArray()
}

internal const val CONTROL_HEADER_LEN = 4

internal fun putIntBE(
    buf: ByteBuffer,
    value: Int,
) {
