...
class DiscoveryRepository @Inject constructor() {
    ...
    suspend fun discoverServers(port: Int, timeoutMs: Int): List<DiscoveredServer> = withContext(Dispatchers.IO) {
        mutex.withLock {
    ...
    suspend fun pair(ip: String, port: Int, deviceId: String, deviceName: String, pin: String): String = withContext(Dispatchers.IO) {
        mutex.withLock {
            SatelliteNative.pair(ip, port, deviceId, deviceName, pin)
        }
    }

    suspend fun connect(ip: String, port: Int, deviceId: String): String = withContext(Dispatchers.IO) {
        mutex.withLock {
            SatelliteNative.httpConnect(ip, port, deviceId)
        }
    }

    suspend fun disconnect(ip: String, port: Int, connectionId: String, deviceId: String): String = withContext(Dispatchers.IO) {
        mutex.withLock {
