...
class WifiConnectionManager @Inject constructor(
    ...
    fun connect(server: DiscoveredServer) {
        ...
        if (existing != null && existing.state.value == WifiState.CONNECTED) {
            existing.updateServer(server)
            return
        }
        val conn = existing ?: WifiConnection(id, server, scope, controllerRepo).also {
    ...
    private suspend fun pairAndConnect(conn: WifiConnection, server: DiscoveredServer) {
        ...
        savePairKey(pair.sharedKey)
        openSession(conn, server)
    }

    fun pairWithPin(server: DiscoveredServer, pin: String) {
        val id = WifiConnection.idFor(server)
        val conn = _connections.value[id] ?: WifiConnection(id, server, scope, controllerRepo).also {
            _connections.value = _connections.value + (id to it)
        }
        conn.markConnecting()
        scope.launch {
    ...
    private suspend fun openSession(conn: WifiConnection, server: DiscoveredServer) = withContext(Dispatchers.IO) {
        val keyHex = getStoredPairKey()
        if (keyHex.length != 64) {
            conn.markDisconnected()
        ...
        if (token.size != 4 || key.size != 32) { conn.markDisconnected(); return@withContext }
        val handle = controllerRepo.openSocket(server.ip, server.udpPort)
        if (handle < 0) { conn.markDisconnected(); return@withContext }
