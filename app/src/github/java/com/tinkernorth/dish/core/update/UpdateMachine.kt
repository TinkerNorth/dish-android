...
internal const val STARTUP_DELAY_MS = 15_000L
internal const val MIN_CHECK_GAP_MS = 60L * 60 * 1000 // 1 h
internal const val FUTURE_SKEW_ESCAPE_MS = 24L * 60 * 60 * 1000 // 24 h
internal const val PERIODIC_INTERVAL_MS = 4L * 60 * 60 * 1000 // 4 h
internal const val BACKOFF_BASE_MS = 10L * 60 * 1000 // 10 min
internal const val BACKOFF_CAP_MS = 6L * 60 * 60 * 1000 // 6 h
internal const val BACKOFF_JITTER = 0.2 // +-20 percent
internal const val MANUAL_MIN_GAP_MS = 10_000L // manual rate limit
internal const val RECONNECT_CHECK_DELAY_MS = 30_000L // after the network returns

// 10, 20, 40 ... 360 min. `failures` is the count INCLUDING the one just
// recorded, so the first failure waits BACKOFF_BASE_MS.
internal fun backoffDelayMs(failures: Int): Long {
