...
2. The gamepad overlay's motion indicator color reflects whatever the real `MotionCapabilityComposer` emits, since that source has not been swapped out. The indicator will read as off / disabled even when `BasicHero` declares gyro enabled on the wired slot. Extracting a `MotionCapabilitySource` interface analogous to the three above would fix it.
